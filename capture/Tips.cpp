...
bool SaveAsBmp( HDC hDC, HBITMAP hBm, char *szFName )
{
	...
	hFile = CreateFile( szFName, GENERIC_WRITE, 0, NULL,
		CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
	if ( hFile == INVALID_HANDLE_VALUE )
	{
		Mes( "ファイルの保存に失敗しました", NULL, MB_OK | MB_ICONERROR );
		return false;
	}

	WriteFile( hFile, &bf, sizeof( BITMAPFILEHEADER ), &dwWritten, NULL );
	WriteFile( hFile, &bi.bmiHeader, sizeof( BITMAPINFOHEADER ), &dwWritten, NULL );
	WriteFile( hFile, lpBits, dwBmSize, &dwWritten, NULL );
...
bool WritePng( char *szFName, LPBYTE lpScBits, int Width, int Height )
{
	...
	lpps = png_create_write_struct( PNG_LIBPNG_VER_STRING, NULL, NULL, NULL );
	if ( !lpps )
	{
		fclose( fp );
		return false;
	...
	if ( !lppi )
	{
		png_destroy_write_struct( &lpps, ( png_infopp )NULL );
		fclose( fp );
		return false;
	}
...
DCSET CreateDCSet( int w, int h )
{
	...
	InitSurface( NULL, DCSet.hDC, DCSet.hBm, w, h );
	DCSet.bSave = FALSE;
